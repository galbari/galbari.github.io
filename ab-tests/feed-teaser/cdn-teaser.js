TRC.executeFeedTeaser = function(options) {
	...
	var config = {
		...
		variantConfig: options.variantConfig || ['organic', 'sponsored'], // items types whcih show on new custom card on desktop - organic or sponsored [AKA - special feed]
	...
	function feedTeaser() {
		...
		function getItmesAsHtmlString(cardsData) {
			var cardsItemsAsHtmlString = cardsData.reduce(function(
				html,
				card,
				index
			) {
		...
		function handleItemClick(e) {
			...
			hideTeaser();
			itemsTypesMap[clickItemType](clickedItem);
		}
      ...
      hideTeaser();
		...
		function handleCardAfterItemMove(cardNode) {
			var itemsList = [].slice.call(
				cardNode.getElementsByClassName('videoCube')
			);
		...
		function moveTitleWithLogoSectionUnderTheTumbnailOfNode(organicNode) {
			...
			if (organicNodeWithTopTitleAndIcon.length) {
				var organicHederSection =
					organicNodeWithTopTitleAndIcon[0].parentElement;
